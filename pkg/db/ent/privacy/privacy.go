...
// The ACLQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type ACLQueryRuleFunc func(context.Context, *ent.ACLQuery) error

// EvalQuery return f(ctx, q).
func (f ACLQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.ACLQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.ACLQuery", q)
}

// The ACLMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ACLMutationRuleFunc func(context.Context, *ent.ACLMutation) error

// EvalMutation calls f(ctx, m).
func (f ACLMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.ACLMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.ACLMutation", m)
}
