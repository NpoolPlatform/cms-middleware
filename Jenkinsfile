    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep cms-middleware | grep $branch
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
          fi
          images=`docker images | grep entropypool | grep cms-middleware | grep none | awk '{ print $3 }'`
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          if [ 0 -eq $rc -a x"$revlist" != x ]; then
            ...
            set +e
            docker images | grep cms-middleware | grep $tag
            rc=$?
            set -e
            if [ 0 -eq $rc ]; then
              DOCKER_REGISTRY=$DOCKER_REGISTRY make release-docker-images
    ...
    stage('Deploy for development') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          sed -i "s/cms-middleware:latest/cms-middleware:$branch/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          if [ "x$REPLICAS_COUNT" == "x" ];then
            REPLICAS_COUNT=2
          fi
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
    ...
    stage('Deploy for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          git reset --hard
          git checkout $tag
          sed -i "s/cms-middleware:latest/cms-middleware:$tag/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          sed -i "s/uhub.service.ucloud.cn/$DOCKER_REGISTRY/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          if [ "x$REPLICAS_COUNT" == "x" ];then
            REPLICAS_COUNT=2
          fi
          sed -i "s/replicas: 2/replicas: $REPLICAS_COUNT/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          sed -i "s/imagePullPolicy: Always/imagePullPolicy: IfNotPresent/g" cmd/cms-middleware/k8s/02-cms-middleware.yaml
          make deploy-to-k8s-cluster
        '''.stripIndent())
      }
    }
